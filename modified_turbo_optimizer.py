...
class ModifiedTurboOptimizer(AbstractOptimizer):
    primary_import = "Turbo"

    def __init__(self, api_config, random_seed=None, **kwargs):
        ...
        self.sobol_engine = torch.quasirandom.SobolEngine(self.turbo.dim, scramble=True, seed=self.random_seed)
    ...
    def init_grid(self):
        ...
        for param in self.space_x.param_list:
            ...
            if isinstance(space, bayesmark.space.Real):
                param_value[param] = list(np.linspace(space.lower, space.upper, 10))
    ...
    def observe(self, X, y):
        ...
        if self.turbo.length < self.turbo.length_min:
            print('Restarting because length is too small')
            self.restart()
        ...
        print('Restarting because all observation is the same')
        self.restart()